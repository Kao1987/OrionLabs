    ...
    <div class="stats-comparison">
      <div class="comparison-grid">
        ...
        >
          <div class="stat-name">
            {{ analysis.statName }}
            <span v-if="analysis.isMainStat" class="main-stat-badge">主屬性</span>
          </div>
...
function getRatingText(rating: RatingLevel): string {
  const textMap: Record<RatingLevel, string> = {
    excellent: '頂級',
    good: '優秀',
    average: '良好',
    poor: '普通',
  ...
  return textMap[rating] || '普通'
}

function getOverallRatingText(rating: OverallRating): string {
  const textMap: Record<OverallRating, string> = {
    godTier: '神級寵物',
    highQuality: '優質寵物',
    normalPet: '普通寵物',
    needImprovement: '待加強',
